...
// Coin 创建一种名为Coin的结构体，包含币的数量，矿工序号，币的时间戳
type Coin struct {
	Num        int64
	MinerIndex int64
	Time       int64
...
// NewCoin 生成新coin函数，传入矿工序号，返回一个coin
...
// InitCoins 初始化coins数组函数，调用NewCoin函数，生成一个coin，然后将其添加到coins数组中
...
// AddCoin 传入新coin和coins数组，将其添加到coins数组中并保存，无返回值
